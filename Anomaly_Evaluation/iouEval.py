...
class iouEval:
    ...
    def addBatch(self, x, y):   #x=preds, y=targets
        ...
        if (x.size(1) == 1):
          x_onehot = torch.zeros(x.size(0), self.nClasses, x.size(2), x.size(3), dtype=torch.float32)  
          if x.is_cuda:
        ...
        if (self.ignoreIndex != -1):
          y = torch.clamp(y, 0, self.nClasses - 1)
          if (y.size(1) == 1):
            y_onehot = torch.zeros(y.size(0), self.nClasses, y.size(2), y.size(3), dtype=torch.float32)
            if y.is_cuda:
